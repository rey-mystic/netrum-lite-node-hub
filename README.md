...
# Default (if installed via nvm):
NETRUM_PATH=$HOME/.nvm/versions/node/v20.19.4/bin/netrum

# Or, if installed globally (npm -g):
# NETRUM_PATH=$(which netrum)

# Path to mining log file
# Default (if you followed the setup below)
...
```
This will keep miner running in background and write logs to ~/netrum-lite-node-hub/mining.log.   
  
Default:
```
$HOME/netrum-lite-node-hub/mining.log
```
